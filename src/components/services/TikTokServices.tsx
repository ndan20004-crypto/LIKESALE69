...
export default function TikTokServices() {
  ...
  const service = (serviceCatalog.tiktok as any)[serviceType || 'like'];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const totalPrice = selectedPackage 
      ? serviceType === 'livestream'
        ? Math.ceil((Number(formData.quantity) * Number(formData.duration) * 9) / 1)
        : Math.ceil((selectedPackage.price * Number(formData.quantity)))
      : 0;
    ...
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-3xl text-white mb-2 flex items-center gap-3">
          {service.icon && <service.icon className="size-8 text-pink-400" />}
          {service.title}
        </h1>
