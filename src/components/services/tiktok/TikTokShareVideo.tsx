...
interface TikTokShareVideoProps {
  userBalance: number;
  onOrderCreated: () => void;
}

export default function TikTokShareVideo({ userBalance, onOrderCreated }: TikTokShareVideoProps) {
  ...
  const servers = [
    { id: 'server1', name: 'Tăng Share Video Beta Server 1', price: 14, min: 1000, max: 100000 },
  ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newOrder = {
        id: `ORD${Date.now()}`,
        service: 'TikTok - Tăng Share Video',
  ...
  return (
    <div className="bg-slate-800 rounded-lg p-6">
      <div className="flex items-center gap-3 mb-6">
        <Share2 className="size-6 text-orange-500" />
        <h2 className="text-xl text-white">Tăng Share Video TikTok</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {servers.map(s => (
              <option key={s.id} value={s.id}>
                {s.name} - {s.price}đ/share (Min: {s.min.toLocaleString()})
              </option>
            ))}
          </select>
        </div>
        ...
        <div className="flex items-start gap-2 bg-orange-500/10 border border-orange-500/20 rounded-lg p-3">
          ...
          <div className="text-sm text-orange-300">
            ...
            <ul className="list-disc list-inside space-y-1 text-xs">
              ...
              <li>Số lượng tối thiểu: 1,000 share</li>
        ...
        <button
          ...
          className="w-full bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-700 hover:to-red-700 disabled:from-slate-600 disabled:to-slate-600 text-white py-3 rounded-lg transition-all flex items-center justify-center gap-2"
        >
          {isSubmitting ? (
            <>
